...
read-only and counter-scoped, so it may be embedded through a `NEXT_PUBLIC_*` variable; the writable
`COUNTERS_API_KEY` used by [`app/counters.ts`](./app/counters.ts) must remain server-only. A publishable
token does not make arbitrary counters public: writes and reads outside its scope fail with HTTP 403.
